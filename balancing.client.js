...
function checkHealth() {
	if(terminating) return;
		...
		console.error(`WARNING: No heartbeat from matchers for ${config.INACTIVITY_TIMEOUT / 1000} seconds`);
		reconnect();
	}
}
...
function cleanExpiredCallbacks(){
	for(let i = 0; i < matcherCallbacks.length; i++){
		if(matcherCallbacks[i].expire >= Date.now()) continue;

		// Notify timeout to the caller
		matcherCallbacks[i].callback({ result: 'timeout' });
		matcherCallbacks.splice(i, 1);
		i--;
	}
}
...
function gotResponse(responseBuffer) {
	lastActivity = Date.now();
	if(!responseBuffer)
		return console.error('ERROR: Empty responseBuffer from matcher');
	...
	try {
		...
		if(!responseParameters.id) {
			console.error('ERROR: Response from matcher is missing Request ID', responseParameters);
		}

		for(let i = 0; i < matcherCallbacks.length; i++){
			if(matcherCallbacks[i].id != responseParameters.id) continue;

			// notify and clean
			matcherCallbacks[i].callback(responseParameters.response);
			matcherCallbacks.splice(i, 1);
			break;
		}
	}
